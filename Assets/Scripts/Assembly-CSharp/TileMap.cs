...
public class TileMap
{
    ...
    public static void paintTilemapLuoi(mGraphics g)
    {
        ...
        for (int j = GameScr.gssx; j < GameScr.gssxe; j++)
        {
            for (int k = GameScr.gssy; k < GameScr.gssye; k++)
            {
                if (maps[k * tmw + j] != 0 && ((!tileTypeAt(j * 24, (k + 1) * 24, 2) && !tileTypeAt(j * 24, (k + 2) * 24, 2) && !tileTypeAt(j * 24, k * 24, 2)) || tileTypeAt(j * 24, k * 24, 2)))
                {
                    if (j > 0)
                    {
                        g.drawRect(j * size, k * size + 8, 24, 24);
                    }
                    else
                    {
                        g.fillRect(j * size, k * 8, 24, 24);
                    }
                }
            }
        }
