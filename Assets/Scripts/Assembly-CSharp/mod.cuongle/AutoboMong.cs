...
{
    public class AutoboMong : IActionListener
    {
    	...
    	public void nvTrainQuai()
    	{
    		try
    		{
    			...
    			{
    				int zoneId = RandomZoneFrom2To10();
    				while (TileMap.zoneID <= 1 && autoboMong)
    				{
    	...
    	public static string GetMapModID(string NameMob)
    	{
    		...
    		foreach (MobInfo mobInfo in array2)
    		{
    	...
    	public void nvTrainVang()
    	{
    		trainVang = true;
    		int num = 10;
    		if (Char.myCharz().taskMaint.taskId < 22)
    		{
    			num = 10;
    		}
    		if (Char.myCharz().taskMaint.taskId == 22 || Char.myCharz().taskMaint.taskId == 23)
    		{
    			num = 68;
    		}
    		else if (Char.myCharz().taskMaint.taskId <= 25)
    		{
    			num = 77;
    		}
    		else if (Char.myCharz().taskMaint.taskId > 25)
    	...
    	public static int RandomZoneFrom2To10()
    	{
    		return (int)(DateTime.Now.Ticks % 100000) % 9 + 2;
    	}
    }
}
