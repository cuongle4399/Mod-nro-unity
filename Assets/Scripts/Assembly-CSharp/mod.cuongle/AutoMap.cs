    ...
    public class AutoMap : IActionListener
    {
    	public class NextMap
    	{
    		...
    		public void Enter(Waypoint waypoint)
    		{
    			...
    			{
    				if (!isEntering || mSystem.currentTimeMillis() - enterDelayStart < 150)
    				{
    					return;
    				}
    				...
    				else if (TileMap.mapID != 166 && MapID != 155)
    				{
    					TeleportTo(num, maxY);
    	...
    	{
    		if (Char.myCharz().meDead && GameCanvas.gameTick % 250 == 0)
    		{
    		...
    		if (!isXmaping || flag || mSystem.currentTimeMillis() - lastWaitTime <= 300 || GameCanvas.gameTick % 30 != 0)
    		{
    			return;
    		}
    		bool flag2 = true;
    	...
    	public static void UpdateXmap(int mapID)
    	{
    		...
    		if (isUseCapsule)
    		{
    			...
    			if (isUsingCapsule && !isOpeningPanel && (GameCanvas.panel.mapNames == null || mSystem.currentTimeMillis() - lastTimeOpenedPanel < 500))
    			{
    				return;
    			}
    			...
    			else
    			{
    				...
    				foreach (NextMap item in linkMaps[num])
    				{
    					...
    					if (num == 19 && mapID2 == 109 && !flag)
    					{
    						...
    						{
    							hashSet.Add(mapID2);
    							int[] array6 = new int[array.Length + 1];
    							Array.Copy(array, array6, array.Length);
    							array6[array.Length] = mapID2;
    							queue.Enqueue(array6);
    					...
    					{
    						hashSet.Add(mapID2);
    						int[] array7 = new int[array.Length + 1];
    						Array.Copy(array, array7, array.Length);
    						array7[array.Length] = mapID2;
    						queue.Enqueue(array7);
    						dictionary[mapID2] = num;
    					}
    				}
    			}
    	...
    	private static string GetErrorMessage(int mapID, List<int[]> validPaths, long currentPower)
    	{
    		...
    		if (!flag)
    		{
    			...
    			{
    				int[] array6 = validPath3;
    				foreach (int num5 in array6)
    				{
    					bool flag3 = false;
    					int[] array7 = idMapsKhiGas;
    					int[] array8 = array7;
