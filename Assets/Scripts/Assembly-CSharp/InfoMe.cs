...
public class InfoMe
{
    ...
    public void addInfo(string s, int Type)
    {
        ...
        if (AutoMap.isXmaping && s.Equals("Bạn chưa thể đến khu vực này"))
        {
            AutoMap.FinishXmap();
        }
        ...
        try
        {
            string text = s.Replace("  ", " ").ToLower().Trim();
            if (text == "nhiệm vụ đã hoàn thành nói chuyện với bò mộng để nhận thưởng" && !FinishBoMong)
            {
                FinishBoMong = true;
            }
            else if (text.Contains(AutoboMong.checkInfo))
            {
                NhanTinHieu = true;
                if (AutoboMong.typeNV(text) == 0 && !AutoboMong.trainning && !AutoboMong.nextnvQuai)
                {
                    nameMap = text.Substring(text.IndexOf("hạ") - 3, text.IndexOf("địa điểm") - text.IndexOf("hạ") + 6);
                    new Thread(AutoboMong.getInstance().nvTrainQuai).Start();
                }
                else if (AutoboMong.typeNV(text) == 1 && !AutoboMong.trainVang && !AutoboMong.nextnvVang)
                {
                    if (AutoboMong.chooseTypeGod)
                    {
                        new Thread(AutoboMong.getInstance().nvTrainVang2).Start();
                    }
                    else
                    {
                        new Thread(AutoboMong.getInstance().nvTrainVang).Start();
                    }
                }
                else if (AutoboMong.typeNV(text) == 2 && !AutoboMong.killCharing && !AutoboMong.nextnvNguoi)
                {
                    new Thread(AutoboMong.getInstance().nvPemNguoi).Start();
                }
                else if ((AutoboMong.typeNV(text) == 3 && !AutoboMong.trainning && !AutoboMong.killCharing && !AutoboMong.trainVang) || (AutoboMong.typeNV(text) == 1 && AutoboMong.nextnvVang) || (AutoboMong.nextnvNguoi && AutoboMong.typeNV(text) == 2) || (AutoboMong.nextnvQuai && AutoboMong.typeNV(text) == 0))
                {
                    new Thread(AutoboMong.huyNV).Start();
                }
            }
            else if (text.Equals(AutoboMong.checknv))
            {
                NhanTinHieu = true;
                EndNvBoMong = true;
