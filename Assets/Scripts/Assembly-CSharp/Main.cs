...
public class Main : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Time.timeScale = ModProCuongLe.timeScale;
        if (Thread.CurrentThread.Name != "Main")
        {
            Thread.CurrentThread.Name = "Main";
        }
        mainThreadName = Thread.CurrentThread.Name;
        if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.IPhonePlayer || Application.isMobilePlatform)
        {
            GameScr.isAnalog = 1;
            isPC = false;
