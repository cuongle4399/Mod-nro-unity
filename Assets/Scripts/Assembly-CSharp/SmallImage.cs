...
public class SmallImage
{
    ...
    public static void drawFlagSquare(mGraphics g, int cFlag, int x, int y)
    {
        if (cFlag != 0)
        {
            int[] flagColors = new int[8] { 13260, 16711680, 6684774, 13421568, 26112, 13382536, 13403136, 5263440 };
            if (cFlag < 1 || cFlag > 8)
            {
                Cout.println("Invalid cFlag value: " + cFlag + ", using default color");
                cFlag = 1;
            }
            int color = flagColors[cFlag - 1];
            float alpha = 0.5f;
