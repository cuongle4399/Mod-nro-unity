...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public static string linkweb = "http://dragonbabe.online";
	...
	public override void paint(mGraphics g)
	{
		...
		if (GameCanvas.currentDialog == null)
		{
			if (!loadScreen)
			{
				if (!bigOk)
				{
                    if (DoHoa.imgLogo != null)
                    {
                        g.drawImage(DoHoa.imgLogo, GameCanvas.hw, GameCanvas.hh - 32, 3);
                    }
                    if (!isGetData)
					{
			...
			else
			{
				...
				if (DoHoa.imgLogo != null)
				{
					g.drawImage(DoHoa.imgLogo, GameCanvas.hw, num4, 3);
				}
				for (int i = 0; i < cmd.Length; i++)
				{
					cmd[i].paint(g);
